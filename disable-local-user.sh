...
usage() {
    ...
    echo '  -r  Removes the home directory associated with account(s).' >&2
    echo '  -a  Creates an archive of the home directory associated with account(s).' >&2
    exit 1
}
...
for USERNAME in "${@}"; do
    ...
    if [[ "${ARCHIVE_OPTION}" = 'true' ]]; then
        # Make sure the ARCHIVE_DIR exists.
        if [[ ! -d "${ARCHIVE_DIR}" ]]; then
            echo "Creating the ${ARCHIVE_DIR} directory."
            mkdir -p "${ARCHIVE_DIR}" || { echo "The archive directory ${ARCHIVE_DIR} could not be created." >&2; exit 1; }
        fi
