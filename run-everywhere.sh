...
SERVER_LIST='/home/chinmaya/shellscript/servers'
...
usage() {
        ...
        echo '  -v       Varbose mode. Displays the server name before executing COMMAND.' >&2
        exit 1
}
...
if [[ ! -e "${SERVER_LIST}" ]]
then
        echo "Can not server list file ${SERVER_LIST}." >&2
        exit 1
fi

# Except the best, prepare the worst.
EXIT_STATUS='0'
...
do
        ...
        then
                ...
                # Capture ant non-zero exit status from the SSH_COMMAND and report to the user.
